    ...
    internal class CActPerfDrumsGauge : CActPerfCommonGauge
    {
        ...
        public override int OnUpdateAndDraw()
        {
            ...
            if (!base.bNotActivated)
            {
                ...
                {
                    // notation view puts the gauge and its SPEED badge along the very bottom
                    int nGaugeTop = CDTXMania.ConfigIni.bDrumsNotationView ? CActPerfDrumsNotation.GAUGE_Y : 626;
                    base.txフレーム.Drums.tDraw2D(CDTXMania.app.Device, base.n本体X.Drums, (CDTXMania.ConfigIni.bReverse.Drums ? 28 : nGaugeTop), new Rectangle(0, 0, base.txフレーム.Drums.szImageSize.Width, 47));
                    ...
                    base.txハイスピ.tDraw2D(CDTXMania.app.Device, -37 + base.n本体X.Drums + base.txフレーム.Drums.szImageSize.Width, (CDTXMania.ConfigIni.bReverse.Drums ? 35 : nGaugeTop + 8), new Rectangle(0, speedTexturePosY, 42, 48));
                    if (base.db現在のゲージ値.Drums == 1.0 && base.txフルゲージ != null)
                    {
                        base.txフルゲージ.tDraw2D(CDTXMania.app.Device, 20 + base.n本体X.Drums, (CDTXMania.ConfigIni.bReverse.Drums ? 37 : nGaugeTop + 9), new Rectangle(0, 0, base.txフレーム.Drums.szImageSize.Width - 63, 31));
                    }
                    else
                    {
                        base.txゲージ.vcScaleRatio.X = (float)base.db現在のゲージ値.Drums;
                        base.txゲージ.tDraw2D(CDTXMania.app.Device, 20 + base.n本体X.Drums, (CDTXMania.ConfigIni.bReverse.Drums ? 37 : nGaugeTop + 9), new Rectangle(0, 0, base.txフレーム.Drums.szImageSize.Width - 63, 31));
                    }
                    base.txフレーム.Drums.tDraw2D(CDTXMania.app.Device, base.n本体X.Drums, (CDTXMania.ConfigIni.bReverse.Drums ? 28 : nGaugeTop), new Rectangle(0, 47, base.txフレーム.Drums.szImageSize.Width, 47));
                }
                /*
                if (base.IsDanger(EInstrumentPart.DRUMS) && base.db現在のゲージ値.Drums >= 0.0)
                {
                    this.txマスクD.tDraw2D(CDTXMania.app.Device, base.n本体X.Drums, (CDTXMania.ConfigIni.bReverse.Drums ? 28 : nGaugeTop));
                }
                if (base.db現在のゲージ値.Drums == 1.0)
                {
                    this.txマスクF.tDraw2D(CDTXMania.app.Device, base.n本体X.Drums, (CDTXMania.ConfigIni.bReverse.Drums ? 28 : nGaugeTop));
                }
                 */
            }
            return 0;
